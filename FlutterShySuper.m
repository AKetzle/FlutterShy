...
% Supersonic
sqrt1 = mu .* r_bar.^2 .* sqrt((RAS_Mach>1.01).*(RAS_Mach.^2 - 1)) ./ (x_bar .* b);
...
flutterPoint_sub = zeros(9,size(mu,1));
iters = size(mu,1);
for i = 1:iters
    [V_f_sub(i), flutterPoint_sub(:,i)] = TR496TR685(freq_alpha, freq_h, a_h, x_bar, r_bar, b, mu(i), invkstepsize, invkMax, g_h, g_alpha);
    V_f_sub(i) = V_f_sub(i) .*(RAS_Mach(i)<=1.01);
end
M_f_sub1 = V_f_sub ./ a;
...
function [Uf, flutterPoint] = TR496TR685(freq_alpha, freq_h, a_h, x_alpha, r_alpha, b, mu, invkstepsize, invkMax, g_h, g_alpha)
    ...
    if(~delta_I_A == 0) % change this to an inline math eq
